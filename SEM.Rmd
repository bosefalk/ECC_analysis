...
dat %<>% rowwise %>% mutate(trst_gov_mean = mean(c(trstprl, trstlgl, trstplc, trstprt)), 
                            trst_ind_mean = mean(c(ppltrst, pplfair, pplhlp)), 
                            pol_news_mean = mean(c(tvpol, rdpol, nwsppol)))
...
sem_means_model <- 'trst_gov_mean ~ trst_ind_mean + pol_news_mean + netuse + hinctnta + agea'
fit_means <- sem(sem_means_model, data = dat_DK4)
summary(fit_means, fit.measures = TRUE)
```


```{r}
library(semTools)
compareFit(fit_DK4, fit_means)

...
SEM_mod2 <- 'trst_gov =~ trstprl + trstlgl + trstplc + trstprt
  trst_ind =~ ppltrst + pplfair + pplhlp
  pol_news =~ tvpol + rdpol + nwsppol
  trst_gov ~ trst_ind + pol_news + netuse + hinctnta

  # Individual trust and news watching is not correlated
  trst_ind ~~ 0*pol_news


  # Income and education is correlated
  #hinctnta ~~ edulvla
...
```{r}
SEM_mod3 <- 'trst_gov =~ trstprl + trstlgl + trstplc + trstprt
  trst_ind =~ ppltrst + pplfair + pplhlp
  pol_news =~ tvpol + rdpol + nwsppol
  trst_gov ~ trst_ind + pol_news + netuse + agea + edulvla + hinctnta

  # Correlations
  edulvla ~~ agea + hinctnta
  agea ~~ hinctnta 
'
